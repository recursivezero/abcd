...
import { Subscribers } from '../models/SubscriptionModel'
...
export const followerList = async (c: Context) => {
  try{
    ...
    const findFollowers = await Subscribers.find({ followingId: userId }).populate('followerId', 'username').select('followerId createdAt');
...
export const followCountAnFollowingCount = async (c:Context) => {
  try{
    ...
    return c.json({message:"Fetched followers and followings count successfully", total:{followers,followings}}, 200);

...
export const queryUsers = async (c: Context) => {
    try {
        ...
        if (from || to) {    // Time range (from and to present)
        ...
        if (order === 'desc') {
            paginatedUsers.sort((a: any, b: any) => new Date(b.timestamp.createdOn).getTime() - new Date(a.timestamp.createdOn).getTime());
        } else {
            paginatedUsers.sort((a: any, b: any) => new Date(a.timestamp.createdOn).getTime() - new Date(b.timestamp.createdOn).getTime());
        }
        if (skip >= total) {
            return c.json({
              message: "Page number exceeds available data.",

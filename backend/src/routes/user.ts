...
userRouter.openapi(
  {
    ...
    request: {
      query: z.object({
        from: z.string().optional().openapi({
          example: "01-09-2024",
          description:
            "Start date (DD-MM-YYYY, YYYY-MM-DD, or days ago as number). If a number, interpreted as days ago. Used with `to` for time range filtering."
        }),
        to: z.string().optional().openapi({
          example: "10-09-2024",
          description:
            "End date (DD-MM-YYYY, YYYY-MM-DD, or days ago as number). Defaults to today if not provided. Used with `from` for time range filtering."
        }),
        since: z.string().optional().openapi({
          example: "7",
          description:
            "Since date (DD-MM-YYYY, YYYY-MM-DD, or days ago as number, e.g., since=7 for last 7 days). Mutually exclusive with `from`/`to`."
        }),
    ...
    responses: {
      ...
      400: {
        description:
          "Invalid or missing parameters. Returned if date formats are wrong, or if from > to, or if page number/size is invalid."
      },
      401: {
        description: "Unauthorized"
      }
